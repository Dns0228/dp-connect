...
{
    if (path == QStringLiteral("starter-guide")) {
        return QStringLiteral("https://github.com/Dns0228/dp-connect/blob/main/docs/SELF_HOSTING.md");
    }

    return QStringLiteral("https://github.com/Dns0228/dp-connect");
}

QString LanguageUiController::getCurrentDocsUrl(const QString &path) const
{
    const QString errorPrefix = QStringLiteral("troubleshooting/error-codes/");
    if (path.startsWith(errorPrefix)) {
        const qsizetype fragmentIndex = path.indexOf('#');
        const QString fragment = fragmentIndex >= 0 ? path.mid(fragmentIndex) : QString();
        return QStringLiteral("https://github.com/Dns0228/dp-connect/blob/main/docs/ERROR_CODES.md") + fragment;
    }

    return QStringLiteral("https://github.com/Dns0228/dp-connect/tree/main/docs");
}
