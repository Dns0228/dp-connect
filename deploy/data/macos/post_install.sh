...
if [ -d "/Applications/${APP_NAME}.localized" ]; then
  ...
  run_cmd mv "/Applications/${APP_NAME}.localized/${APP_NAME}.app" "$APP_PATH"
  run_cmd rm -rf "/Applications/${APP_NAME}.localized"
fi
...
if dscl . -read "/Groups/$SERVICE_GROUP" >/dev/null 2>&1; then
  ...
  run_cmd dscl . -create "/Groups/$SERVICE_GROUP" RealName "DP Connect Service Group"
fi
...
run_cmd osascript -e 'tell application "DPConnect" to quit' || true
...
if [ -f "$PLIST_SOURCE" ]; then
  ...
  echo "ERROR: service plist not found at $PLIST_SOURCE" >&2
  exit 1
fi
